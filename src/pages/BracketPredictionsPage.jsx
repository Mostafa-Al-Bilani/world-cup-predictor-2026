...
export function BracketPredictionsPage() {
    ...
    try {
      ...
      const nextDrafts = Object.fromEntries(
        STAGE_PREDICTION_CONFIGS.map((stage) => [
          stage.key,
          predictionRows.find((prediction) => prediction.stage === stage.key)?.selected_teams ?? [],
        ]),
      );
      ...
      toast.error(getSafeErrorMessage(error, 'Could not load bracket predictions.'));
    } finally {
      setLoading(false);
    }
  }, [user.id]);
  ...
  const toggleTeam = (stage, team) => {
    const config = STAGE_PREDICTION_CONFIGS.find((item) => item.key === stage);
    setDrafts((current) => {
      const selected = current[stage] ?? [];
      if (selected.includes(team)) {
        return { ...current, [stage]: selected.filter((item) => item !== team) };
      }
      if (selected.length >= config.requiredCount) {
        toast.error(`Select exactly ${config.requiredCount} teams. Remove one before adding another.`);
        return current;
      }
  ...
  const saveStage = async (stage) => {
    ...
    try {
      ...
      setPredictions((current) => [saved, ...current.filter((prediction) => prediction.stage !== saved.stage)]);
      setDrafts((current) => ({ ...current, [saved.stage]: saved.selected_teams ?? [] }));
      toast.success(`${STAGE_PREDICTION_CONFIGS.find((item) => item.key === stage)?.label} prediction saved.`);
    } catch (error) {
      toast.error(getSafeErrorMessage(error, 'Could not save this bracket prediction.'));
    } finally {
      setSavingStage('');
    }
  };

  if (loading) return <LoadingSpinner label="Loading bracket predictions" />;

  return (
    <main className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-sm font-black uppercase tracking-[0.32em] text-emerald-300">Bracket predictions</p>
          <h1 className="mt-3 text-4xl font-black sm:text-5xl">Pick who advances.</h1>
          <p className="mt-4 max-w-3xl text-slate-300">
            Predict which teams reach each knockout stage before that round begins. These points are separate from
            match predictions and your existing World Cup winner pick.
          </p>
        </div>
        <div className="rounded-lg border border-gold-300/30 bg-gold-300/10 p-4 text-sm text-gold-100 lg:max-w-sm">
          ...
          <p className="mt-1 text-slate-200">
            Your champion pick is still worth 3 points: {championPrediction?.predicted_team ?? 'not selected yet'}.
          </p>
        </div>
      </div>

      <section className="mt-8 rounded-lg border border-emerald-300/30 bg-emerald-300/10 p-5">
        <p className="font-black text-emerald-100">Bracket predictions are optional.</p>
        <p className="mt-1 text-sm text-slate-200">
          Save each stage before its first kickoff. Missing a stage does not block you from using matches, groups, or
          the scoreboard.
        </p>
      </section>
      ...
      {!availableTeams.length ? (
        ...
        <div className="mt-8 space-y-6">
          {STAGE_PREDICTION_CONFIGS.map((stage) => {
            ...
            const status = stageStatus({ locked, prediction });
            const canEdit = !locked && !prediction?.scored_at;
            const isComplete = selected.length === stage.requiredCount;

            return (
              <section key={stage.key} className="overflow-hidden rounded-lg border border-white/10 bg-slate-950/72">
                <div className="border-b border-white/10 bg-white/[0.03] p-5">
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        <span className={`rounded-full border px-3 py-1 text-xs font-black uppercase ${statusStyles[status]}`}>
                          {status}
                        </span>
                      ...
                      <h2 className="mt-3 text-2xl font-black text-white">{stage.label}</h2>
                      <p className="mt-2 max-w-3xl text-sm text-slate-300">{stage.description}</p>
                    </div>
                    <div className="grid gap-2 text-sm sm:grid-cols-3 lg:min-w-[420px]">
                      <Metric label="Each correct" value={`${stage.pointsPerTeam} pts`} />
                      <Metric label="Max points" value={stage.requiredCount * stage.pointsPerTeam} />
                      <Metric label="Earned" value={prediction?.points_awarded ?? 0} />
                    </div>
                  </div>
                  <div className="mt-4 flex flex-col gap-2 text-sm text-slate-300 sm:flex-row sm:items-center sm:justify-between">
                    <span className="inline-flex items-center gap-2">
                      ...
                      {lockAt ? `Locks ${formatDateTime(lockAt)}` : 'Locks when this stage begins'}
                    </span>
                    <span className="inline-flex items-center gap-2">
                ...
                <div className="p-5">
                  <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    {availableTeams.map((team) => {
                      ...
                      const disabled = !canEdit || (!active && selected.length >= stage.requiredCount);
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="truncate text-sm font-black">{team}</span>
                        </button>
                      );
                    })}
                  </div>
                  ...
                  {prediction?.scored_at ? (
                    <div className="mt-5 rounded-lg border border-gold-300/30 bg-gold-300/10 p-4 text-sm text-gold-100">
                      <p className="font-black">
                        Scored {prediction.correct_count ?? 0}/{stage.requiredCount} correct for {prediction.points_awarded ?? 0} points.
                      </p>
                      {prediction.correct_teams?.length ? (
                        <p className="mt-1 text-slate-200">Correct: {prediction.correct_teams.join(', ')}</p>
                      ) : null}
                    </div>
                  ) : null}
                  ...
                  <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <button
                      type="button"
                      disabled={!canEdit || !isComplete || savingStage === stage.key}
                    ...
                    >
                      {savingStage === stage.key ? <Trophy size={16} className="animate-pulse" /> : <Save size={16} />}
