...
const resultOptions = [
  { value: "", label: "Not set" },
  { value: "team_a", label: "Home/listed team wins" },
  { value: "draw", label: "Draw" },
  { value: "team_b", label: "Away/opponent team wins" },
];
...
export function AdminMatchForm({ match, onSubmit, onCancel, saving }) {
  ...
  const updateForm = (event) => {
    ...
    setForm((current) => {
      ...
      if (name === "team_a_score" || name === "team_b_score") {
        next.result =
          getMatchResultFromScores(next.team_a_score, next.team_b_score) ??
          next.result;
      }
  ...
  return (
    <form
      onSubmit={submit}
      className="rounded-lg border border-white/10 bg-slate-950/72 p-5"
    >
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        ...
        <Field
          ...
          helpText={
            savedUtcPreview
              ? `Saved as ${formatDateTimeUtc(savedUtcPreview)} in Supabase.`
              : "Choose a local kickoff time."
          }
          required
        />
