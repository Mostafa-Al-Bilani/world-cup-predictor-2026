...
export const VALID_MATCH_STATUSES = new Set([
  "upcoming",
  "live",
  "halftime",
  "extra_time",
...
export function normalizeText(
  value,
  { label = "Value", required = false, maxLength = 120 } = {},
) {
...
export function normalizePredictionScore(value, label) {
  ...
  if (!Number.isInteger(number) || number < 0 || number > 99) {
    throw new Error(
      `${label} must be a non-negative whole number from 0 to 99.`,
    );
  }
...
export function normalizePredictionScorePair(homeScore, awayScore) {
  const predictedHomeScore = normalizePredictionScore(
    homeScore,
    "Home team score",
  );
  const predictedAwayScore = normalizePredictionScore(
    awayScore,
    "Away team score",
  );
...
export function normalizeMatchPayload(match) {
  const normalizedId = match.id
    ? validateUuid(match.id, "Match ID")
    : undefined;
  ...
  const payload = {
    ...match,
    team_a: normalizeText(match.team_a, {
      label: "Home/listed team",
      required: true,
      maxLength: limits.teamName,
    }),
    team_b: normalizeText(match.team_b, {
      label: "Away/opponent team",
      required: true,
      maxLength: limits.teamName,
    }),
    ...
    stage: normalizeText(match.stage, {
      label: "Stage",
      required: true,
      maxLength: limits.stage,
    }),
    status,
    team_a_score: normalizeNonNegativeInteger(
      match.team_a_score,
      "Home/listed score",
    ),
    team_b_score: normalizeNonNegativeInteger(
      match.team_b_score,
      "Away/opponent score",
    ),
    result,
    venue:
      normalizeText(match.venue, { label: "Venue", maxLength: limits.venue }) ||
      null,
    city:
      normalizeText(match.city, { label: "City", maxLength: limits.city }) ||
      null,
    host_country:
      normalizeText(match.host_country, {
        label: "Host country",
        maxLength: limits.hostCountry,
      }) || null,
  };
